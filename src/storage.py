...
def ensure_output_dir(base_path: str | Path, run_date: date) -> Path:
    """Create and return the dated output directory."""
    base = Path(base_path)
    dir_name = analysis_dir_name(run_date)
...
def write_daily_positions(
    output_dir: Path, snapshot: DailyPositionsSnapshot
) -> Path:
    """Write the daily enriched positions snapshot."""
    filename = dated_filename("daily-positions", snapshot.date, "yaml")
...
def write_raw_articles(
    ...
    filename = dated_filename("raw_articles", run_date, "json")
    filepath = output_dir / filename
...
def write_ranked_articles(
    ...
    filename = dated_filename("ranked_articles", run_date, "json")
    filepath = output_dir / filename
...
def write_digest(output_dir: Path, run_date: date, content: str) -> Path:
    """Write the Markdown digest."""
    filename = dated_filename("market_digest", run_date, "md")
    filepath = output_dir / filename
...
def write_summary_payload(
    output_dir: Path, run_date: date, payload: dict
) -> Path:
    """Write the summary payload JSON."""
    filename = dated_filename("summary_payload", run_date, "json")
    filepath = output_dir / filename
...
def write_run_log(
    output_dir: Path, run_date: date, log_lines: list[str]
) -> Path:
    """Write the run log."""
    filename = dated_filename("run_log", run_date, "txt")
    filepath = output_dir / filename
