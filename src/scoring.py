...
def score_for_general_market(
    articles: list[NormalizedArticle],
    source_priorities: dict[str, int] | None = None,
) -> list[ScoredArticle]:
    """Score articles for General Market Update mode.

    No position matching — scores purely on source priority and recency.
    Returns ScoredArticle list sorted by score descending.
    """
    source_priorities = source_priorities or {}
    scored: list[ScoredArticle] = []

    for article in articles:
        source_priority = source_priorities.get(article.source, 5)
        priority_bonus = source_priority * SOURCE_PRIORITY_FACTOR * 3  # weight priority more
        ...
        total = round(priority_bonus + recency_bonus, 3)
        scored.append(
            ScoredArticle(
                article=article,
                position_scores={},
    ...
    if scored:
        logger.info(
            f"General market scored {len(scored)} articles, top score: {scored[0].portfolio_score}"
        )
    else:
        logger.info("No articles to score (general market mode)")
